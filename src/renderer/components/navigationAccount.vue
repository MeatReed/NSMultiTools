<template>
  <div>
    <v-navigation-drawer app permanent expand-on-hover right>
      <v-list-item v-if="!$store.state.authUser" class="px-2">
        <v-list-item-avatar>
          <v-img :src="`file:///${__resources}/account.png`" />
        </v-list-item-avatar>
      ...
      <v-list v-if="!$store.state.authUser" dense>
        <v-list-item link>
          <v-list-item-icon>
      ...
      <v-list v-else dense>
        ...
        <v-list-item v-if="$store.state.authUser.role === 'admin'" link>
          <v-list-item-icon>
    ...
    <v-dialog v-model="dialogLogin" width="500">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-checkbox v-model="checkboxRememberMe" label="Se souvenir de moi" />
        </v-card-text>
...
export default {
  ...
  computed: {
    passwordConfirmationRule() {
      return () =>
        this.passwordRegister === this.passwordConfirmRegister ||
  ...
  methods: {
    ...
    async register() {
      if (
        ...
        if (
          /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(this.emailRegister)
        ) {
