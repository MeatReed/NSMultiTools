  ...
  <div>
    ...
    <v-content>
      ...
      <v-container class="fill-height lighten-5" fluid>
              ...
              <v-row>
                <v-col>
                  <v-text-field
                    ...
                    @input="inputUpdate"
                  />
                  <v-alert :value="valueAlert" :type="typeSerial">
                    {{ messageSerial }}
                  </v-alert>
                </v-col>
              </v-row>
        ...
        <v-row>
          <v-col>
            <v-btn @click.stop="dialogSN = true">
              Où trouver son numéro de série ?
            </v-btn>
          </v-col>
          <v-col>
            <v-btn nuxt to="/inject">
              Injecter un payload sur sa Nintendo Switch
            </v-btn>
          </v-col>
        </v-row>
        <v-dialog v-model="dialogSN">
          <v-card>
            ...
            <v-card-text>
              Il y a trois façons de trouver le numéro de série de sa Nintendo
              Switch
              <ol>
                <li>
                  Sur l'emballage de la console situé sur la face inférieure.
                </li>
                <li>
                  Sur la console situé en bas.<v-img
                    :src="require('../assets/serie.png')"
                    aspect-ratio="2"
                  />
                </li>
                <li>
                  Ou dans les paramètres de la console en allant dans
                  <kbd>Console</kbd> et ensuite <kbd>Numéro de série</kbd>
                </li>
              </ol>
            </v-card-text>
            ...
            <v-card-actions>
              ...
              <v-btn text @click="dialogSN = false">
                Fermer
              </v-btn>
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    send_data(serial_input, prefix, serial_digits, status) {
      this.messageSerial = status
      if (status === 'safe') {
        this.valueAlert = true
        this.messageSerial =
          "Votre Nintendo Switch n'est pas patchée, vous pouvez correctement la hacker !"
        ...
        this.valueAlert = true
        this.messageSerial =
          'Le numéro de série que vous avez saisi est incorrect !'
        this.typeSerial = 'error'
      }
    },
    check(serials, serial) {
      ...
      if (serial_input.length >= 4) {
        ...
        if (serial_input.length <= 10)
          return this.send_data(
            serial_input,
            first_part,
            serial_input.slice(3, 10)
          )

        if (isNaN(second_part))
          return this.send_data(serial_input, first_part, second_part)
