            ...
            <v-row>
              <v-col>
                ...
                <div :class="showRedeemCode ? '' : 'd-none'">
                  ...
                  />
                  <v-btn v-on:click="manualRetrieve()" :disabled="btnRetrieveDisabled" :loading="btnRetrieveLoading">Activer sa licence</v-btn>
                </div>
                </v-col>
            </v-row>
          </v-col>
  ...
  methods: {
    ...
    sign_csr(csr_data, code) {
      ...
      .catch(function (error) {
        self.messageAlert = "Une erreur est survenue, votre licence n'est sûrement pas valide ou les serveurs SX XECUTER ne sont pas fonctionnels."
    ...
    get_license(csr_data, code) {
      ...
      .then(function (response) {
        ...
        for(var i=0; i<response.data.license.length/2; i++) {
          license_file[i] = parseInt(response.data.license.substr(i*2,2),16);
        }
