        ...
        <v-row>
          <v-col>
            <v-app-bar
              dense
            >
              <v-toolbar-title>Catégorie {{ categoryName[category] }}</v-toolbar-title>
              <v-spacer></v-spacer>
          ...
          <v-col v-for="(item, index) of packages" :key="index">
            ...
            >
              ...
              <v-card-actions>
                ...
                <v-btn
                  text
                  v-on:click="modalPackage(packages ,item.name)"
                >
                  Information
                </v-btn>
              </v-card-actions>
...
export default {
  ...
  created() {
    try {
      axios.get('https://www.switchbru.com/appstore/repo.json').then(async response => {
