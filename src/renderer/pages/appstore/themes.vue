  ...
  <div>
    ...
    <v-content>
      <v-container fluid>
        <v-row>
          <v-col>
            <v-app-bar
              dense
            >
              <v-toolbar-title>{{ $t('appstore.category') }} {{ $t('appstore.themes') }}</v-toolbar-title>
              <v-spacer></v-spacer>
        ...
        <v-row>
          <v-col v-for="(item, index) of themesPackages" :key="index">
            <v-card
              class="mx-auto"
              max-width="344"
              outlined
            >
              ...
              <v-card-actions>
                ...
                <v-btn
                  text
                  v-on:click="modalPackage(themesPackages ,item.name)"
                >
...
export default {
  ...
  async asyncData({ $axios }) {
    try {
      ...
      //Games
      const themesPackages = new Array;
      await data.packages.forEach((element, index) => {
        if(element.category === "theme") {
          themesPackages.push(element)
        }
      });

      return {
        themesPackages: themesPackages
      }
    } catch (e) {
      console.log(e)
    }
