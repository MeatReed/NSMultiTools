  ...
  <div>
    ...
    <v-content>
      <v-container fluid>
        <v-row>
          <v-col>
            <v-app-bar
              dense
            >
              <v-toolbar-title>{{ $t('appstore.category') }} {{ $t('appstore.emulators') }}</v-toolbar-title>
              <v-spacer></v-spacer>
        ...
        <v-row>
          <v-col v-for="(item, index) of emuPackages" :key="index">
            <v-card
              class="mx-auto"
              max-width="344"
              outlined
...
export default {
  ...
  async asyncData({ $axios }) {
    try {
      ...
      //Games
      const emuPackages = new Array;
      await data.packages.forEach((element, index) => {
        if(element.category === "emu") {
          emuPackages.push(element)
        }
      });
